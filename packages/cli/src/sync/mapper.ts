...
export function mapStatsRecordToSyncRecord(record: StatsRecord): SyncRecord {
  ...
  // OpenCode and Hermes records are sourced from SQLite DBs; lineOffset=0 is a sentinel with no
  // positional meaning, so generateSyncRecordId would produce the same ID for every
  // record from the same DB. Use record.id directly since it already encodes the unique row/session ID.
  const id = record.sourceFile.startsWith('synced/')
    ? record.id
    : record.tool === 'opencode' || record.tool === 'hermes'
      ? record.id

...
program
  ...
  .option('--tool <tool>', 'Filter by tool type (claude-code|codex|openclaw|opencode|hermes|qoder|cursor|copilot)')
  .action((options) => {
    const db = createDatabase(DB_PATH)
    const state = getState(AIUSAGE_DIR)
    const summary = generateSummary(db, {
...
program
  ...
  .option('--tool <tool>', 'Specific tool to parse (claude-code|codex|openclaw|opencode|hermes|qoder|cursor|copilot)')

...
import { getAllConfigs, setConfig, getDefaultValue, CONFIG_DESCRIPTIONS, CONFIG_DESCRIPTIONS_ZH, CONFIG_CATEGORIES, CONFIG_UNITS, CFG } from '$lib/server/config.js'
...
export const GET: RequestHandler = async (event) => {
  ...
  const configs: Record<string, { value: number; default: number; description: string | null; description_zh: string | null; unit: string | null; unit_zh: string | null }> = {}
...
export const PUT: RequestHandler = async (event) => {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    await setConfig(key, value, description, admin.id)
    updated.push(key)
  }

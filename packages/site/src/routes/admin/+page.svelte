              ...
              {#each users as user}
                <div class="list-item">
                  ...
                  <div class="list-item-actions">
                    ...
                    {/if}
                    {#if user.role === 'user'}
                      <button class="btn btn-ghost" on:click={() => setRole(user.id, user.username, 'admin')} disabled={!!actionLoading}>{zh ? '设为管理员' : 'Make admin'}</button>
                    {:else if user.role === 'admin'}
                      <button class="btn btn-ghost" on:click={() => setRole(user.id, user.username, 'user')} disabled={!!actionLoading}>{zh ? '取消管理' : 'Demote'}</button>
                    {/if}
                  </div>
                </div>
              {/each}

...
- **工具调用提取：** Codex 的工具调用与 token 计数是独立的 JSONL 行，需两步关联：
  ...
  - 若会话结束时队列仍有未关联的工具调用（无后续 token_count），则丢弃并写入 `warnings.log`
...
```json
{
  ...
  "syncConsentTarget": "sha256(github|username/aiusage-data|api.github.com|global|ts,inputTokens,outputTokens,cacheReadTokens,cacheWriteTokens,thinkingTokens,cost,tool,model,provider,sessionKey,device,deviceInstanceId,updatedAt|read,write|schema:v1)[0:16]",
...
**日志轮转：** `errors.log`、`warnings.log` 和 `audit.log` 各自达到 10MB 时，将当前文件重命名为 `*.log.1`（保留 1 个备份），新建空文件继续写入；轮转后新文件需保持原有最小权限。
...
| `aiusage serve` | 启动时同步解析一次；之后每 **60 秒** 后台增量解析（若上次未完成则跳过） | 启动时先同步执行一次本地过期清理；清理完成后再启动 HTTP 服务，并在服务就绪后后台异步同步一次 |
...
**解析互斥锁（PID 锁）：** 解析开始时在 `~/.aiusage/parse.lock` 写入当前进程 PID。获取锁前检查文件是否存在：若 PID 对应进程仍在运行（Unix 用 `kill -0 <pid>`，Windows 用 `tasklist /FI "PID eq <pid>"`）则等待最多 5 秒；若进程已不存在（崩溃残留），直接清除旧锁并继续。超时仍无法获取锁则跳过解析、使用已有数据并输出提示。解析完成后删除锁文件。
...
! 将上传脱敏统计数据：时间、token、cost、模型名、AI 助手类型、sessionKey、device、deviceInstanceId
! 不上传：对话内容、代码、提示词、文件内容、绝对路径、工具调用详情
! 本次审批绑定目标：<resolved-target>（schema v1）
! 本次审批绑定字段清单：ts,inputTokens,outputTokens,cacheReadTokens,cacheWriteTokens,thinkingTokens,cost,tool,model,provider,sessionKey,device,deviceInstanceId,updatedAt
...
使用 **绝对路径 + 行字节起始偏移** 作为哈希输入，在本机范围内全局唯一且与解析运行次数无关：

```
id = sha256(sourceFile + lineOffset)[0:16]  // 前 16 位 hex
```
...
interface StatsRecord {
  ...
  ingestedAt: number                   // 写入本地数据库时间（毫秒），用于同步增量游标
  syncedAt?: number                    // 最近一次成功上传到云端的时间（毫秒）；未同步时为空
  updatedAt: number                    // 最近一次解析/重算该记录业务字段的时间（毫秒），用于远端增量上传
...
interface ToolCallRecord {
  id: string                           // sha256(recordId + name + ts + callIndex) 前 16 位 hex
  recordId: string                     // 关联的 StatsRecord.id
...
interface SyncRecord {
  id: string                           // 稳定身份键；默认复用对应 StatsRecord.id
  ts: number
  tool: 'claude-code' | 'codex' | 'openclaw'
  model: string
  provider: string
  ...
  cost: number
  sessionKey: string                   // sha256(device + sessionId)[0:16]，用于同设备内会话聚合，避免上传原始 sessionId
  device: string                       // 展示用途的设备别名，不作为“当前设备副本”判定依据
  deviceInstanceId: string             // 稳定设备实例 ID，用于识别当前设备副本和设备作用域
  updatedAt: number                    // 最近一次生成该 SyncRecord 的时间（毫秒）
}
...
> 3. `SyncRecord.id` 必须是稳定身份键，不能包含 token、cost、model 等可能因解析器修复或价格表更新而变化的业务值。v1 默认复用来源 `StatsRecord.id`，它保证**同一设备内幂等**，但**不保证跨设备识别“同一原始日志副本”**；因此产品语义明确为“按设备汇总使用统计”，不尝试合并不同设备上可能重复出现的同一日志副本。
...
每次解析完成后，将已处理进度写入 `~/.aiusage/watermark.json`，使用 **字节偏移** 而非行号。路径存储为绝对路径：

```json
{
  "claude-code": {
    "/Users/alice/.claude/projects/xxx/abc123.jsonl": { "offset": 28456, "size": 28456, "mtime": 1776738085346 }
  },
  "codex": {
    "/Users/alice/.codex/sessions/2026/05/12/rollout-abc123.jsonl": { "offset": 15320, "size": 15320, "mtime": 1776738085000 }
  },
  "openclaw": {
    "/Users/alice/.openclaw/agents/main/sessions/xxx.jsonl": { "offset": 9840, "size": 9840, "mtime": 1776738085000 }
  }
}
```
...
**文件身份识别：** watermark 除 `offset/size/mtime` 外，还必须保存平台可用的稳定文件身份信息。Unix/macOS 优先使用 `(dev, ino)`；Windows 使用文件 ID（如 volume serial + file index）。若平台无法稳定获取文件身份，则至少回退保存文件开头固定字节窗口的内容指纹；身份校验失败时必须保守地按“文件已替换”处理，禁止直接从旧 `offset` 续读。

**水位线清理：** 每次解析时，删除 watermark 中指向已不存在文件的条目，防止长期膨胀。

**clean 命令与水位线联动：** `aiusage clean --before Nd` 删除本地数据库记录后，对受影响文件将 watermark 重置为 0，确保再次解析时不产生数据缺口。该操作仅影响本地解析状态，不直接删除云端数据；传 `--remote` 时另走远端清理流程。
...
CREATE TABLE records (
  id                TEXT PRIMARY KEY,   -- sha256(sourceFile + lineOffset) 前 16 位 hex
  ts                INTEGER NOT NULL,
  ingested_at       INTEGER NOT NULL,   -- 首次写入本地数据库时间（同步增量游标）
...
CREATE TABLE synced_records (
  id                TEXT PRIMARY KEY,   -- SyncRecord.id
  ts                INTEGER NOT NULL,
  tool              TEXT NOT NULL,
  model             TEXT NOT NULL,
  provider          TEXT NOT NULL,
  ...
  device            TEXT NOT NULL,
  device_instance_id TEXT NOT NULL,
  updated_at        INTEGER NOT NULL    -- 最近一次从远端拉取或本地映射生成该记录的时间
);
...
CREATE TABLE tool_calls (
  id          TEXT PRIMARY KEY,         -- sha256(record_id + name + ts + callIndex) 前 16 位 hex
...
CREATE INDEX idx_records_synced     ON records(synced_at DESC);
...
Dashboard 前端通过 **轮询**（默认 30 秒间隔）刷新数据。
...
**`export` 数据模型约定：**
- `csv` 和 `json` 默认导出面向本地分析的 `StatsRecord` 视图，包含时间、AI 助手、模型、token、cost、sessionId、device 等字段；不导出 `sourceFile`、`lineOffset`
- `ndjson` 明确导出 `SyncRecord`，用于备份、校验或与云端主数据文件做比对；它**不**包含 `SyncTombstone`，因此不等同于完整云端同步状态
...
已知模型定价：6 个模型
未知定价模型：deepseek-v3（出现 12 次，cost 记为 0）
```
...
**隐私声明：同步到云端的数据仅为脱敏后的统计记录（时间、token 数、成本、模型名、AI 助手类型、`sessionKey`、device、`deviceInstanceId`），不含工具调用名称、对话内容、代码、提示词、文件内容、绝对路径、原始 `sessionId` 或日志字节偏移。设备名称默认使用主机名，可在 `config.json` 中通过 `device` 字段自定义别名。`deviceInstanceId` 为本地生成的稳定随机标识，用于避免将不同设备的同名别名误判为同一设备。尽管不包含正文内容，设备别名、设备实例标识、时间分布和模型分布仍可能构成敏感元数据，因此首次远端访问前必须进行显式确认。**
...
3. **上传阶段：** 从本地 `records` 中筛选 `synced_at IS NULL OR updated_at > synced_at` 的记录，映射为 `SyncRecord` 后，只有当其命中 `deviceScope="*"` 或 `deviceScope === SyncRecord.deviceInstanceId` 的 tombstone 时才禁止上传；其他设备作用域的 tombstone 不得阻止该记录上传。上传成功后将这些本地记录的 `synced_at` 更新为当前同步时间
...
- 本地记录如因解析器修复、价格表更新或补算逻辑变化而发生字段更新，应保持 `SyncRecord.id` 不变，仅刷新 `updatedAt` 和业务字段
...
- 月文件超过 5MB 时，后续记录按日存储：`data/YYYY/MM/DD.ndjson`；月文件保留已有内容
- tombstone 文件超过 5MB 时，后续记录按日存储：`data/YYYY/MM/DD.tombstones.ndjson`
...
- `init` 默认只授予 `read + write`；首次执行 `clean --remote` 前，必须显式升级审批为包含 `delete` 的操作集合，升级后写回新的 `syncConsentAt/syncConsentTarget`
...
- 云端数据默认不自动清理，由用户自行管理
- `sync_tombstones` 与远端 tombstone 文件在 v1 不自动清理；若后续引入压实策略，必须证明不会导致离线设备重新上传已删除记录后方可删除 tombstone
...
- **单元测试**：每个解析器用真实 JSONL fixture 文件做快照测试（三个 AI 助手各有真实日志样本）；provider 推断、价格计算、id 生成（sourceFile + lineOffset 哈希）、callIndex 赋值、OpenClaw cost=0 与字段缺失的分支均需覆盖

...
1. 用户可以在 `aiusage.jtanx.com` 注册、登录并查看 Token 天梯榜。
...
| 角色 | 能力 |
|---|---|
| 游客 | 访问官网首页和文档；不能查看排行榜详情 |
| 普通用户 | 注册、登录、绑定 OAuth、授权 CLI、上传数据、查看排行榜 |
...
1. 用户运行 `aiusage leaderboard login`。
...
1. 用户在新设备运行 `aiusage leaderboard login`。
...
- 如果用户后续还需要在旧设备上传，必须在旧设备重新运行 `aiusage leaderboard login`，生成新的设备记录和新的设备密钥。
...
| `device_revoked` | 设备已撤销 | 提示重新执行 `aiusage leaderboard login` |
...
- `device_not_found`、`device_revoked`：提示重新执行 `aiusage leaderboard login`。
...
用户可以查看自己的上传状态。CLI 的 `aiusage leaderboard status` 和站点设置页应展示：
...
- 使用 cursor-based 分页（基于 `total_tokens` 和 `user_id` 组合游标），避免 offset 在大数据量下的性能问题。
- 响应包含 `next_cursor` 字段，客户端传入 `cursor` 参数获取下一页。
- 除分页列表外，额外返回当前登录用户的排名和数据（即使不在当前页内），方便用户了解自己的位置。
...
| `PUBLIC_SITE_URL` | URL | 站点公开地址，如 `https://aiusage.jtanx.com` |
